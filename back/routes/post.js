...
router.post('/', isLoggedIn, upload.none() , async (req,res,next)=> {
    try{
        ...
        if(req.body.image){
            console.log('!레큐바디이미지',req.body.image);
...
router.post('/:id/like', isLoggedIn , async( req, res, next) => {
    try{
        ...
        console.log('!래큐유저아이디', req.user.id);
...
router.post('/:id/retweet' , isLoggedIn , async (req,res,next) => {
    try{
        ...
        if(exPost){
            return res.status(403).send('이미리스트에 있습니다');
        }
        ...
        const retweetWithPrevPost = await db.Post.findOne({
            ...
            include: [{ model: db.User , attributes: ['id', 'nickname']},
                      { model: db.Post , as: 'Retweet', 
                                include: [{ model: db.User, attributes: ['id', 'nickname']},{model: db.Image}]
                    }]
        });
